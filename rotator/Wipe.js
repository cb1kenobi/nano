define([
	...
	function _setClip(/*DomNode*/n, /*int*/type, /*int*/w, /*int*/h, /*number*/x){
		...
		style.set(n, "clip", type == null ? "auto" : "rect(" + _clipArray(type, w, h, x).join("px,") + "px)");
	}
	...
	function _wipe(/*int*/type, /*Object*/args){
		...
		style.set(node, {
			display: "",
			zIndex: (style.get(args.current.node, "zIndex") || 1) + 1
		});

		_setClip(node, type, w, h);

		return new baseFx.Animation(lang.mixin({ /*dojo.Animation*/
			node: node,
			curve: [0, type % 2 ? w : h],
