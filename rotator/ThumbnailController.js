define([
	...
	'dojo/dom-attr',
	'dojo/query'
], function(declare, connect, lang, evt, dom, css, attr, query){
	var _css = "dojoxRotatorThumb",
		_selected = _css + "Selected";

	return declare("nano.rotator.ThumbnailController", null, {
		// summary:
		...
		// example:
		//	|	<div dojoType="nano.Rotator" jsId="myRotator">
		...
		//	|	</div>
		//	|	<div dojoType="nano.rotator.ThumbnailController" rotator="myRotator"></div>
			...
			if(r){
				...
				for(var i=0; i<r.panes.length; i++){
					var n = r.panes[i].node,
						s = attr.get(n, "thumbsrc") || d.attr(n, "src"),
						t = attr.get(n, "alt") || "";
