define([
	...
	,'dojo/Deferred'
	,'dojo/on'
], function(declare, lang, baseFx, array, connect, parser, dom, domConstruct, style, attr, geom, query, topic, Deferred, on){

	// build friendly strings
	var _defaultTransition = "nano.rotator.swap", // please do NOT change
	...
	lang.setObject(_defaultTransition, function(/*Object*/args){
		...
		return new baseFx.Animation({ // dojo.Animation
			play: function(){
				style.set(args.current.node, _displayStr, _noneStr);
				style.set(args.next.node, _displayStr, "");
				this._fire("onEnd");
			}
		});
	});
	
	return declare("nano.Rotator", null, {
		// summary:
		...
		//		- /*nano.Rotator*/ rotator - the rotator instance
		...
		//	|		dojo.require("nano.rotator.Fade");
		//	|	</script>
		//	|	<div dojoType="nano.Rotator" transition="nano.rotator.crossFade">
		//	|		<div>Pane 1!</div>
		//	|		<div>Pane 2!</div>
		//	|	</div>
		...
		constructor: function(/*Object*/params, /*DomNode|string*/node){
			...
			var _t = this,
				...
				cb = _t._domNodeContentBox = geom.getContentBox(node), // we are going to assume the rotator will not be changing size
			...
			if(style.get(node, "position") == "static"){
				style.set(node, "position", "relative");
			}
			...
			query(">", node).forEach(function(n, i){
				var q = { node: n, idx: i, params: lang.mixin({}, tp, eval("({ " + (attr.get(n, "transitionParams") || "") + " })")) },
					r = q.trans = attr.get(n, "transition") || _t.transition;
				...
				if(_t.idx == null || attr.get(n, "selected")){
					if(_t.idx != null){
						style.set(pp[_t.idx].node, _displayStr, _noneStr);
					}
					_t.idx = i;
					p.display = "";
				}
			...
			_t._controlSub = topic.subscribe(_t.id + "/rotator/control", _t, "control");
		},

		destroy: function(){
			// summary:
			//		Destroys the Rotator and its DOM node.
			array.forEach([this._controlSub, this.wfe], topic.unsubscribe);
		...
		go: function(/*int|string?*/p){
			...
			style.set(current.node, _zIndex, 2);
			style.set(next.node, _zIndex, 1);
			...
			if(anim){
				...
				var def = new Deferred(),
					...
					h = connect.connect(anim, "onEnd", function(){
						// reset the node styles
						style.set(current.node, {
							display: _noneStr,
							left: 0,
							opacity: 1,
							top: 0,
		...
		resize: function(/*int*/width, /*int*/height){
			...
			geom.setContentSize(this._domNode, b);
			array.forEach(this.panes, function(p){ geom.setContentSize(p.node, b); });
		},
